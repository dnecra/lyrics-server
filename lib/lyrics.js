...
async function getTranslationCacheEntry(videoId) {
    ...
    try {
        ...
        const raw = await fsp.readFile(filePath, 'utf8');
        const parsed = JSON.parse(raw);
        if (!parsed || typeof parsed.timestamp !== 'number' || !isTranslationPayload(parsed.data)) {
            return null;
        }
        const entry = {
            timestamp: parsed.timestamp,
...
function isLikelyNonEnglishLyricsData(data) {
    ...
    const hasStrongNonEnglishLatinSignal = (text) => {
        const normalizedText = ` ${String(text || '').toLowerCase()} `;
        if (!normalizedText.trim()) return false;

        if (/[áéíóúüñ¿¡àèìòùâêîôûãõç]/i.test(normalizedText)) {
            return true;
        }

        const strongWordPatterns = [
            /\b(?:cariño|corazon|corazón|solo|sola|pienso|sueño|sueno|hecho|realidad|pero|juntos|junto|mundo|tiempo|pasando)\b/i,
            /\b(?:oye|hola|adios|adiós|gracias|por favor|te quiero|mi amor|mi vida)\b/i,
            /\b(?:el|la|los|las|un|una|unos|unas|del|al|que|de|en|con|sin|para|por|como|cuando|donde|porque)\b/i
        ];
    ...
    if (eligibleLines.some((line) => hasStrongNonEnglishLatinSignal(line))) {
        return true;
    }
    ...
    if (detectionCounts.size === 0) return false;
    ...
    return !['en', 'eng'].includes(topLang);
}

async function attachTranslation(videoId, data) {
    ...
    if (!payload || !Array.isArray(payload.translatedSyncedLyrics) || payload.translatedSyncedLyrics.length === 0) {
        return {
            ...data,
            translationState: isLikelyNonEnglishLyricsData(data) ? 'pending' : 'skipped'
        };
    }
    return {
        ...data,
...
async function attachCachedTranslation(videoId, data) {
    const cached = await getTranslationCacheEntry(videoId);
    if (!cached || !isTranslationPayload(cached.data)) {
        return {
            ...data,
...
function queueTranslationGeneration(videoId, data, baseResponseData, broadcast, meta = {}) {
    if (!videoId || !data || activeTranslationFetches.has(videoId)) return;
    if (!isLikelyNonEnglishLyricsData(data)) {
    ...
    const job = (async () => {
        try {
            const translatedResponse = await attachTranslation(videoId, baseResponseData);
...
async function attachTranslationForDelivery(videoId, data) {
    if (!data) return data;
    if (isLikelyNonEnglishLyricsData(data)) {
        return attachTranslation(videoId, data);
    }
    return attachCachedTranslation(videoId, data);
}
...
async function fetchLyrics(videoId, artist, title, album = '', duration = 0, broadcast) {
    ...
    if (cached) {
        ...
        if (!hasSyncedInCache && cached.data && cached.data.plainLyrics) {
            ...
            responseData = await attachTranslationForDelivery(videoId, responseData);
...
async function fetchLyricsCandidate(videoId, artist, title, album = '', duration = 0, candidateOffset = 0, broadcast) {
    ...
    if (!data || (!data.syncedLyrics && !data.plainLyrics)) {
        return {
            success: false,
            error: 'No lyrics candidate found',
